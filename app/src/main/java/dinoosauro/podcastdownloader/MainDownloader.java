...
public class MainDownloader extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences preferences = getSharedPreferences(getBaseContext().getPackageName(), Context.MODE_PRIVATE);
        if (preferences.getBoolean("KeepScreenOn", false)) {
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        }
        AppCompatDelegate.setDefaultNightMode(preferences.getInt("DefaultNightMode", -1)); // Update dark mode. "-1" is "Follow system settings".
    }
